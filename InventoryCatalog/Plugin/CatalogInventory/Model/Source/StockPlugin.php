...
class StockPlugin
{
     ...
     * @param string $dir
     * @return \Magento\CatalogInventory\Model\Source\Stock
     */
    public function aroundAddValueSortToCollection(
        \Magento\CatalogInventory\Model\Source\Stock $subject,
        callable $proceed,
        $collection,
        string $dir
    ) {
        $proceeded = false;
        if ($collection instanceof Collection && $collection->getStoreId() !== Store::DEFAULT_STORE_ID) {
        ...
        return !$proceeded ? $proceed($collection, $dir) : $subject;
    }
}
